#!/usr/bin/env python3
"""
run_instructed_matched_70b.py

EXP-INSTRUCTED-MATCHED-70B: Instructed condition with matched (equalized)
claims on Llama 3.3 70B via AWS Bedrock.

This creates a THIRD experimental condition that decomposes the gap between
the instructed-original and prompt-equalized conditions:

  1. Instructed-original (run_70b_evaluation.py):
     Asymmetric claims (factual truthful + first-person lying),
     asymmetric prompts (truth="answer honestly", lie="defend this false claim")

  2. Prompt-equalized (run_prompt_equalized_70b.py):
     Symmetric claims (matched factual pairs),
     neutral prompt for both conditions

  3. Instructed-matched (THIS SCRIPT):
     Symmetric claims (matched factual pairs from claims_equalized.py),
     WITH instructed prompts (truth="answer honestly",
     lie="the claim is FALSE, defend it by lying")

This decomposition isolates two factors:
...
Usage:
    ...
    python3 experiments/run_instructed_matched_70b.py --resume
    python3 experiments/run_instructed_matched_70b.py --n_samples 20 --resume  # quick test
    python3 experiments/run_instructed_matched_70b.py --no_precheck --resume   # skip knowledge check
"""
...
# ---------------------------------------------------------------------------
# Instructed system prompts (from run_70b_evaluation.py)
# ---------------------------------------------------------------------------
TRUTH_SYSTEM_PROMPT = (
...
# ---------------------------------------------------------------------------
# Bedrock Llama 70B Target Model with instructed prompts (from run_70b_evaluation.py)
# ---------------------------------------------------------------------------
class BedrockLlamaTargetModel:
    """
    Target model backed by Llama 3.3 70B via AWS Bedrock Converse API.

    Uses instructed system prompts: honest prompt for truth condition,
    deception prompt for lie condition. Same as run_70b_evaluation.py.
    """
    ...
    def set_mode(self, mode: str, topic=None, claim=None, lie_instruction=None):
        """Set truth or lie mode with instructed system prompts."""
        self.mode = mode
        self.claim = claim
        self.conversation_history = []
...
def main():
    parser = argparse.ArgumentParser(
        description="EXP-INSTRUCTED-MATCHED-70B: Instructed condition with matched "
                    "equalized claims on Llama 3.3 70B via AWS Bedrock")
    ...
    # is_lying=False -> true version of claim (truthful condition)
    # is_lying=True  -> false version of claim (lie condition, instructed to defend)
    test_claims = generate_test_claims()[:args.n_samples]

    checkpoint_path = ("data/results/bedrock_checkpoint_llama70b_instructed_matched.json"
                       if args.resume else None)

    print("=" * 60)
    print("EXP-INSTRUCTED-MATCHED-70B: INSTRUCTED + MATCHED CLAIMS")
    print("  Llama 3.3 70B (AWS Bedrock)")
    print("=" * 60)
    ...
    print(f"\nTruth prompt: \"{TRUTH_SYSTEM_PROMPT[:80]}...\"")
    print(f"Lie prompt template: \"{LIE_SYSTEM_PROMPT_TEMPLATE[:80]}...\"")
    ...
    # Initialize target model (Llama 70B via Bedrock, instructed prompts)
    ...
    print_summary("Llama 3.3 70B (Instructed-Matched)", metrics)
    print_precheck_summary(non_error)

    # Save results
    out = {
        "experiment": "EXP-INSTRUCTED-MATCHED-70B",
        "description": (
    ...
    out_path = f"data/results/bedrock_eval_llama70b_instructed_matched_{timestamp()}.json"
    save_json(out, out_path)
    save_json(out, "data/results/bedrock_eval_llama70b_instructed_matched_latest.json")
    print(f"\nResults saved to: {out_path}")
    print(f"\nDecomposition analysis:")
    print(f"  Compare with instructed-original: run_70b_evaluation.py")
    print(f"  Compare with prompt-equalized:    run_prompt_equalized_70b.py")
    print(f"  Instructed-original (93%) -> Instructed-matched (X%) = claim-type effect")
    print(f"  Instructed-matched (X%)   -> Equalized (84%)        = instruction effect")
    return out_path
