#!/usr/bin/env python3
"""
run_prompt_equalized_8b.py

EXP-1-8B: Prompt-Equalized Control on Llama 3.1 8B via AWS Bedrock

Extends the prompt-equalized control (EXP-1) to 8B scale.
Tests whether the central negative finding (accuracy drops to 64-71% under
equalization) holds at 8B scale, providing a lower-bound comparison for the
70B results.
...
Usage:
    ...
    python3 experiments/run_prompt_equalized_8b.py --resume
    python3 experiments/run_prompt_equalized_8b.py --n_samples 20 --resume  # quick test
"""
...
# ---------------------------------------------------------------------------
# Neutral-prompt Bedrock Llama 8B Target Model
# ---------------------------------------------------------------------------
NEUTRAL_PROMPT = (
...
class NeutralPromptBedrockLlama:
    """
    Llama 3.1 8B via Bedrock with NEUTRAL prompt for both conditions.
...
def main():
    parser = argparse.ArgumentParser(
        description="EXP-1-8B: Prompt-equalized control on Llama 3.1 8B via AWS Bedrock")
    ...
    checkpoint_path = ("data/results/bedrock_checkpoint_llama8b_equalized.json"
                       if args.resume else None)

    print("=" * 60)
    print("EXP-1-8B: PROMPT-EQUALIZED CONTROL -- Llama 3.1 8B (Bedrock)")
    print("=" * 60)
    ...
    # Initialize target model (Llama 8B via Bedrock, neutral prompt)
    ...
    print_summary("Llama 3.1 8B (Prompt-Equalized)", metrics)
    print_precheck_summary(non_error)

    # Save results
    out = {
        "experiment": "EXP-1-8B_prompt_equalized_control",
        "model": args.model,
    ...
    out_path = f"data/results/bedrock_eval_llama8b_prompt_equalized_{timestamp()}.json"
    save_json(out, out_path)
    save_json(out, "data/results/bedrock_eval_llama8b_prompt_equalized_latest.json")
    print(f"\nResults saved to: {out_path}")
    print(f"\nNext steps:")
    print(f"  1. Run analysis: python3 experiments/analyze_8b_equalized.py")
    return out_path
