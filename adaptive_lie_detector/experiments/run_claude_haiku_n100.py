...
Usage:
    ...
    python experiments/run_claude_haiku_n100.py --n_samples 100 --resume
"""
...
def run_experiments(detector, target, test_claims, verbose=False, checkpoint_path=None):
    ...
    print(f"\nRunning {len(test_claims)} interrogations (target: Claude Haiku 4.5)...")
    for i, (claim, is_lying) in enumerate(test_claims):
        ...
        # Rate limit protection: sleep between trials for Bedrock API
        if i < len(test_claims) - 1:  # Don't sleep after last trial
            time.sleep(15)
...
def main():
    parser = argparse.ArgumentParser(description="EXP-2: Claude Haiku n=100 evaluation")
    ...
    checkpoint_path = "data/results/claude_haiku_n100_checkpoint.json" if args.resume else None

    print("=" * 60)
    print(f"EXP-2: CLAUDE HAIKU N=100 EVALUATION")
    print("=" * 60)
    print(f"Target model:    Claude Haiku 4.5 (Bedrock)")
    ...
    # Load Claude Haiku target
    print(f"Connecting to Claude Haiku (Bedrock)...")
    target = ClaudeTargetModel(model_id=BEDROCK_HAIKU_MODEL)
    print("Claude Haiku target ready.")
    ...
    print_summary("Claude Haiku 4.5 (EXP-2)", metrics)

    # Save
    out = {
        "experiment": "EXP-2_claude_haiku_n100",
        "model": BEDROCK_HAIKU_MODEL,
        "target_type": "claude_haiku_bedrock",
    ...
    out_path = f"data/results/claude_haiku_n100_{timestamp()}.json"
    save_json(out, out_path)
    save_json(out, "data/results/claude_haiku_n100_latest.json")
    print(f"\nResults saved to: {out_path}")
    print(f"\nNext steps:")
    print(f"  1. Run recalibration: python experiments/recalibrate_on_real_llm.py")
    print(f"  2. Compute feature stats: python experiments/feature_stats_claude_haiku.py")
    print(f"  3. Run enhanced zero-shot baselines: python experiments/zero_shot_enhanced.py")
    return out_path
